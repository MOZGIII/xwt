...
#[derive(Debug)]
pub struct ExampleServer<Connection>
where
    // Require that the connection we accept implements full capabilities
    // of the WebTransport connection.
    ...
    Connection: xwt_core::Connection,
{
    ...
    pub connection_type: core::marker::PhantomData<Connection>,
}

impl<Connection: xwt_core::Connection> Default for ExampleServer<Connection> {
    fn default() -> Self {
...
impl<Connection: xwt_core::Connection> ExampleServer<Connection> {
    /// Handle the inbound connection from a client.
    pub async fn handle(&self, connection: Connection) {
        let client_id = self
            .client_id_counter
            .fetch_add(1, std::sync::atomic::Ordering::Relaxed);

        // Upon connecting, the client will open a chat stream.
        let (mut chat_send, chat_read) = connection.accept_bi().await.unwrap();
        ...
        let movement_loop = self.movement_loop(client_id, &connection);
    ...
    async fn chat_inbound_loop(&self, mut chat_stream: Connection::RecvStream) {
    ...
    async fn chat_outbound_loop(
        &self,
        mut chat_stream: Connection::SendStream,
    ...
    pub async fn movement_loop(&self, client_id: u64, connection: &Connection) {
        loop {
            // Receive the movement datagram from the client.
            // This can fail if the connection closes.
            let movement_datagram = connection.receive_datagram().await.unwrap();
