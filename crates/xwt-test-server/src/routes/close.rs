...
pub async fn accept_and_close_bi_send_streams_with_error_code(
    ...
    loop {
        ...
        if let Err(error) = send_stream.reset(webtransport_code_to_http_code(123)) {
            match error {
                ...
                | wtransport::error::StreamWriteError::QuicProto
                | wtransport::error::StreamWriteError::Stopped(_) => unreachable!(),
            }
        }
    }
}
